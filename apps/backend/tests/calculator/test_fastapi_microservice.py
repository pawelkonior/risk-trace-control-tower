...
class FastApiMicroserviceTests(unittest.TestCase):
    ...
    def test_dashboard_snapshot_endpoint_exposes_frontend_payload(self) -> None:
        response = self.client.get("/v1/dashboard/snapshot")

        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertEqual(payload["currency"], "PLN")
        self.assertGreater(len(payload["metrics"]), 0)
        self.assertGreater(len(payload["topExposures"]), 0)
        self.assertGreater(len(payload["exposureClass"]), 0)
        self.assertIn("capitalSummary", payload)
