from __future__ import annotations

from fastapi import FastAPI, Request
from fastapi.responses import JSONResponse

from rwa_steering.errors import SteeringDomainError

from .engine import RATS_ENGINE_VERSION, RwaRATSService
from .schemas import ApiErrorDetail, ApiErrorResponse, RATSRequest, RATSResponse


def create_app() -> FastAPI:
    """Create the FastAPI app for the Risk-Aware Trading Swarm service."""
    service = RwaRATSService()
    app = FastAPI(
        title="RWA RATS Service",
        version=RATS_ENGINE_VERSION,
        description="Risk-Aware Trading Swarm optimization on forecasted RWA portfolios.",
    )
    app.state.service = service

    @app.exception_handler(SteeringDomainError)
    async def steering_domain_error_handler(
        request: Request, exc: SteeringDomainError
    ) -> JSONResponse:
        """Return stable structured errors for domain and generated-input failures."""
        _ = request
        payload = ApiErrorResponse(
            error=ApiErrorDetail(
                code=exc.detail.code,
                message=exc.detail.message,
                field_path=exc.detail.field_path,
                severity=exc.detail.severity,
                remediation=exc.detail.remediation,
                context=exc.detail.context,
            )
        )
        return JSONResponse(status_code=422, content=payload.model_dump(mode="json"))

    @app.get("/v1/health", tags=["service"])
    @app.get("/health", tags=["service"])
    def health() -> dict[str, str]:
        """Return lightweight liveness metadata for orchestration probes."""
        return {
            "status": "ok",
            "service": "rwa-rats-service",
            "rats_engine_version": RATS_ENGINE_VERSION,
            "input_package_version": app.state.service.input_package.manifest.version_id,
            "input_package_validation_status": (
                app.state.service.input_package.manifest.validation_status
            ),
        }

    @app.post("/v1/rats/optimize", tags=["rats"])
    @app.post("/rats/optimize", tags=["rats"])
    def optimize(request: RATSRequest) -> RATSResponse:
        """Run RATS optimization on a forecasted portfolio slice."""
        return app.state.service.optimize(request)

    return app


app = create_app()
