# RWA Steering Backend

Python backend microservice for Basel III RWA calculation, projection and steering services.
It owns the `src/` layout, Python dependency lockfile, CLI entry points and backend tests:
...
- `src/rwa_dashboard` - backend data/API helpers consumed by the React frontend.

The backend uses a modern `src/` layout, `uv` for environment and lockfile management,
...
From the repository root, the backend is built and run by Docker Compose:

```powershell
docker compose up --build backend
```

Compose starts Postgres first and injects `RWA_DATABASE_URL` into the backend:

```text
postgresql+psycopg://rwa:rwa_local_password@postgres:5432/rwa_steering
```

The container exposes the calculator API on `http://127.0.0.1:8000`. When running the backend
directly on the host against the compose database, use the `127.0.0.1` URL from
`backend/.env.example`.

React dashboard API:

```text
GET http://127.0.0.1:8000/v1/dashboard/snapshot
```

The React frontend in `../frontend` is the only browser UI. It consumes backend REST data for:
...
RiskTrace REST endpoints planned on the backend calculator API:

```text
GET  http://127.0.0.1:8000/v1/app/context
GET  http://127.0.0.1:8000/v1/dashboard/snapshot
GET  http://127.0.0.1:8000/v1/lineage/traces/{trace_id}
GET  http://127.0.0.1:8000/v1/briefing/snapshot
POST http://127.0.0.1:8000/v1/ui/actions/{action_id}
GET  http://127.0.0.1:8000/v1/exports/{export_type}
GET  http://127.0.0.1:8000/v1/notifications
GET  http://127.0.0.1:8000/v1/search
```
